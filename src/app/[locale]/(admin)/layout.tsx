...
export default async function AdminLayout({
    ...
    return (
        <div className="min-h-screen bg-[#FDFCF8] flex flex-col md:flex-row">
            {/* Sidebar for Desktop */}
            <aside className="hidden md:flex w-64 flex-col bg-white border-r border-zinc-100 p-6 fixed h-full z-30">
                <div className="flex items-center gap-3 mb-10 px-2">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] uppercase font-black tracking-widest text-[#25D366]">Admin Platforme</span>
                    </div>
                </div>

                <nav className="flex-1 space-y-2">
                    <NavLink href="/admin" icon={<LayoutDashboard className="w-5 h-5" />} label="Vue d'ensemble" />
                    <NavLink href="/admin/users" icon={<Users className="w-5 h-5" />} label="Utilisateurs" />
                    <NavLink href="/admin/settings" icon={<Settings className="w-5 h-5" />} label="Configuration" />
                </nav>
            ...
            <main className="flex-1 overflow-y-auto md:ml-64">
                <header className="h-20 bg-white border-b border-zinc-200 flex items-center justify-between px-10 sticky top-0 z-10">
                    <h1 className="text-sm font-bold uppercase tracking-widest text-zinc-400">Plateforme de Gestion</h1>
