...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign In' button in the top navigation (element index 78) to open the login page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/nav/div/div[2]/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Navigate to /login (use navigate action to http://localhost:3000/login) to reach the login page and continue authentication flow.
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Type vendor email into the email field (index 312) and proceed to enter password and submit the login form.
        frame = context.pages[-1]
        # Input text
        ...
        await expect(frame.locator('text=Paramètres').first).to_be_visible(timeout=3000)
